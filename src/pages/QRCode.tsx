...
export default function QRCodePage() {
  ...
  const generateQRCode = async () => {
    ...
    try {
      ...
      const newSession = await createSession(evolutionConfig.id, evolutionConfig.instance_name, currentCompany.id);
      
      // Gerar QR Code real baseado nos dados da sessão
      const qrData = `whatsapp-session:${newSession.id}:${currentCompany.id}:${Date.now()}`;
